...
import { Plus, Edit2, Trash2, Check, X, Tag, Layers } from 'lucide-react';
import {
  addInitiative, updateInitiative, deleteInitiative,
} from '../utils/storage';
import { generateId, computeMetrics, statusColor, formatDate } from '../utils/helpers';

const STATUSES = ['active', 'paused', 'completed'];

const DEFAULT_PLATFORMS = ['Reddit', 'LinkedIn', 'Cold Email', 'WhatsApp', 'Community Forums', 'Events', 'In-person', 'Email'];
const DEFAULT_TYPES = ['Post', 'DM', 'Email', 'Demo', 'Follow-up', 'Call'];
...
function InitiativeCard({ initiative, activities, onEdit, onDelete }) {
  const initActivities = activities.filter((a) => a.initiativeId === initiative.id);
  const metrics = computeMetrics(initActivities);
    ...
    <div className="card p-5 space-y-4 hover:border-gray-700 transition-colors">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex gap-1.5 flex-shrink-0">
          ...
          <button onClick={() => onDelete(initiative.id)} className="btn-danger p-1.5"><Trash2 size={13} /></button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-gray-800/50 rounded-lg p-2.5 text-center">
          <div className="text-lg font-bold font-mono text-cyan-400">{metrics.outreach.toLocaleString()}</div>
          <div className="text-xs text-gray-500 mt-0.5">Outreach</div>
        </div>
        <div className="bg-gray-800/50 rounded-lg p-2.5 text-center">
          <div className="text-lg font-bold font-mono text-violet-400">{metrics.responses}</div>
          <div className="text-xs text-gray-500 mt-0.5">Responses</div>
        </div>
        <div className="bg-gray-800/50 rounded-lg p-2.5 text-center">
          <div className="text-lg font-bold font-mono text-emerald-400">{metrics.conversionRate}%</div>
          <div className="text-xs text-gray-500 mt-0.5">Conv. Rate</div>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="card max-w-xl w-full p-6 space-y-5 my-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button onClick={handleSave} disabled={!form.name.trim()} className="btn-primary flex-1 disabled:opacity-50">
...
export default function Initiatives({ initiatives, activities, onDataChange, addToast }) {
